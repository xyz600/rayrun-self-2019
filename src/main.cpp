...
//
typedef void (*PreprocessFun)(const float *vertices, size_t numVerts,
                              const float *normals, size_t numNormals,
                              const uint32_t *indices, size_t numFace);

typedef void (*IsectFun)(Ray *rays, size_t numRay, bool hitany);
...
public:
  ...
  Vec3(float xyz) : x_(xyz), y_(xyz), z_(xyz) {}
  Vec3(const float *ptr) : x_(ptr[0]), y_(ptr[1]), z_(ptr[2]) {}
  ...
  static Vec3 cross(Vec3 rhs, Vec3 lhs) {
    return Vec3(rhs.y_ * lhs.z_ - rhs.z_ * lhs.y_,
...
public:
  ...
  Vec3 world2local(Vec3 world) const {
    return Vec3(Vec3::dot(world, s_), Vec3::dot(world, t_),
                Vec3::dot(world, n_));
  }
  Vec3 local2world(Vec3 local) const {
    return Vec3(Vec3::dot(local, is_), Vec3::dot(local, it_),
                Vec3::dot(local, in_));
  }
...
//
static std::tuple<std::vector<float>, std::vector<float>, std::vector<uint32_t>>
...
public:
  void load(const std::string &filename) {
    ...
    const auto getV3 = [](picojson::value &obj) -> std::array<float, 3> {
      ...
      return {float(posArr[0].get<double>()), float(posArr[1].get<double>()),
...
public:
  ...
  double elapsed() {
    return (double)std::chrono::duration_cast<std::chrono::milliseconds>(end_ -
                                                                         start_)
        .count();
  }
  void print(const char *tag) { printf("%s %4.1fms\n", tag, elapsed()); }
...
void main(int32_t argc, char **argv) {
  ...
  const PreprocessFun preprocess =
      (PreprocessFun)GetProcAddress(dll, "preprocess");
  ...
  swPreprocess.start();
  preprocess(vertices.data(), vertices.size() / 3, normals.data(),
             normals.size() / 3, indices.data(), indices.size() / 6);
  swPreprocess.stop();
...
#pragma omp parallel for schedule(dynamic, 16) reduction(+ : rayCount)
  for (int32_t y = 0; y < height; ++y) {
    ...
    for (int32_t x = 0; x < width; ++x) {
      ...
      for (int32_t np = 0; np < numPrimRay; ++np) {
        ...
        const float xs =
            px * iw * std::tanf(hfovy * float(width) / float(height));
  ...
  stbi_write_png("output.png", setting.width, setting.height, 3,
                 ldrImage.data(), setting.width * 3);
#endif
  //
  swIsect.print("isect");
  const float mrays =
      float(double(rayCount) / double(swIsect.elapsed() * 1000.0));
  printf("%.2fMRays/sec\n", mrays);
  //
  FreeLibrary(dll);
}
